...
We will test our algorithm for non-trivial low-qubit using the IBM-QISKit based local simulator, before trying out 5-qubit ibmqx4 and the 16-qubit ibmqx5 machines. 
...
Next, one maps the time-evolution operator into quantum circuits consisting of 2-qubit gates. The mapping that results in the shortest depth quantum circuit is an open question. Finally, one plugs the quantum circuit for $U$ into the quantum phase estimation algorithm.
Then, using the quantum phase estimation (QPE) algorithm, one can for example compute the eigenvalues of a simple nuclear pairing Hamiltonian, as done in \cite{ovrum2007}. We have recently repeated these calculations in collaboration with MSU Ph.D. student Benjamin Hall, who also works together with MSU Ph.D. student Ryan Larose on related topics. The first near-term goals are to extend this model to include pairing plus particle-hole excitations and study the variational quantum eigensolver (VQE). As stated above, the latter is a hybrid quantum-classical algorithm that is based on the variational principle of quantum mechanics
...
Exploring the QPE and the VQE applied to a simpler Hamiltonian rewritten in terms of various gates, is thus our first step of WP4. Parts of these results have already been presented in \cite{ovrum2007}. The Hamiltonian is rather simple and these results and computations will also link well with the two main topics in WP1. 
Based on these results we extend our ambitions to study the design of quantum  algorithms for the IMSRG and UCC theories. The algorithms we develop will be able to run on both existing quantum computers  and classical computers. 
...
Our first step in these studies, is to apply these algorithms to the nuclear pairing Hamiltonian, with and without particle-hole operators. These are models which have been extensively studied with many classical quantum mechanical algorithms.  
In a recent work Babbush {\em et al} develop the formalism for performing quantum simulations with a plane-wave bases \cite{Babbush:2018wy} for the homogeneous electron gas (HEG). Our final goal is to extend our quantum algorithms for the UCC and IMSRG  to both the HEG and infinite nuclear matter. This will lay the foundation for the application of quantum algorithms for studies of finite nuclei and infinite nuclear matter. 
