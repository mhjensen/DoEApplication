...
%unit of information for a quantum computer is the %so-called qubit. Any suitable two-level quantum %system can be a qubit, but
...
%The aim of this project/proposal is thus to explore and develop algorithms based on quantum computing in order to study nuclear systems. 
%\begin{comment}
%The first aim is to implement these algorithms on %classical
...
The timeline for WP1 is summarized in the table here:
\begin{footnotesize}
\begin{center}
...
\end{figure}
 ...
 The timeline for the development of the various elements of WP2 is
\begin{footnotesize}
\begin{center}
\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|c|}
\hline
...
variational adiabatic method described in Sec.~\ref{sec_VAE}. %~\ref{sssec:vae}. 
...
The timeline for WP3 is summarized in the table here:
...
The main goal of WP4 is to perform quantum simulations of infinite nuclear matter by rewriting classical many-body methods like Coupled Cluster (CC)  and Similarity Renormalization Group (SRG) theory in terms of quantum algorithms. In particular, we will work on the so-called Unitary Coupled Cluster (UCC) and In-Medium SRG (IMSRG) approaches. The UCC method provides a computationally affordable and systematically improvable wave function that is suitable for implementation on current and near-term quantum computers as well as existing and future supercomputers. Our first test case is going to be a simplified nuclear model, the so-called pairing Hamiltonian, before we venture into calculations of the homogeneous electron gas and infinite nuclear matter based on realistic nuclear interactions. These systems have been extensively studied with standard Coupled Cluster theory and IMSRG as well as many other many-body methods.
\tbd{[Does this sound like repetition, then, if results are available? What's new?]}
...
Typical examples of popular many-body methods are the aforementioned CC approach, various types of Monte Carlo methods, perturbative expansions, Green's function methods, the Density-Matrix Renormalization Group (DMRG),
...
complexity, and therefore are very hard to solve for larger systems. In contrast, a so-called quantum computer, a particularly dedicated computer,
...
hardware, and the uniqueness of its capabilities, it is interesting to explore these two aspects from a nuclear many-body  perspective and this constitutes the major motivation of WP4.
...
The timeline for WP4 is summarized in the table here:
\begin{footnotesize}
\begin{center}
\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|c|}
\hline
...
experimental and theoretical post-doctoral fellows and approximately 100 graduate
...
with students, post-doctoral fellows and more senior researches with diverse
backgrounds on a daily basis. 
A few years ago FRIB/NSCL strengthened
and improved the mentoring of graduate students and post-doctoral fellows at the
FRIB/NSCL with the appointment of an Associate Director for
Education. A formal half-year review of all post-doctoral fellows was initiated. It
...
with all FRIB/NSCL post-doctoral fellows on a monthly basis to keep them informed
...
group. post-doctoral fellows also maintain their own website, where they share
experiences, hints and collect and list job openings. post-doctoral fellows are
...
post-doctoral fellows have been participating in an MSU sponsored Work/Life balance
...
events offer additional interaction of current students and post-doctoral fellows
...
offered to be contacted by students and post-doctoral fellows for career advice.  The list can be
filtered by profession and geographic distribution.  Finally, the
post-doctoral fellows are an integral part of all laboratory social activities at
the FRIB/NSCL which include the Tuesdays coffee and bagel, Thursdays ice
...
The timeline for WP5 is summarized in the table here:
\begin{footnotesize}
\begin{center}
\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|c|}
\hline
...
career paths.  Furthermore, the competences and skills acquired by the Ph.D. candidates and postdoctoral fellows in computational science, quantum computing and quantum information theories will lay the foundation for the education of the next generation of the scientific workforce. 
...
\includepdf[page=-]{support/Combined_CPS.pdf}
